...
# react-md with-react-router-v4
This project was bootstrapped with [Create React App](https://github.com/facebook/create-react-app).

This project followed all the steps laid out in the [with-create-react-app example](../with-create-react-app), so please follow
the steps outlined there before continuing on.

## Adding react-router
First, let's install `react-router`:
```sh
$ yarn add react-router-dom
$ yarn add react-router   # this one is optional
```

Next, let's move the default `src/App.js` children into `src/Home.js`:
```diff
...
and create the `src/Home.js` file:
```diff
+import React, { Component } from 'react';
+import './home.css';
+import logo from './logo.svg';
...
and move the styles from `src/App.scss` to `src/Home.scss`:
```sh
$ mv src/App.scss src/Home.scss
```

Next, lets create a few pages that can be imported. Create `src/Page1.js`:
```diff
+import React, { Component } from 'react';
...
One of the problems with using `react-md` and `react-router` together is that most of the components in `react-md` are `PureComponent`s
to help with performance. This is not the best with `react-router` since they rely on `this.context` updates in components to be able to update.
`PureComponent`s only render when `this.props` or `this.state` has ben updated, so it can stop the rendering process for `react-router` since
the components will not update on `this.context` changes. The way to get around this is to create a custom wrapper for the `Link` component
and to always do an inline map of a list of nav items in the `render` function.

Let's start with the custom `Link` wrapper by creating `src/NavLink.js`:
```diff
...
This will allow you to correctly apply the `active` style to each `ListItem` when a route is active, and conditionally
add `FontIcon` to left side of each `ListItem`.

Next, let's update te `src/App.js` to include our routes and `react-router`:
```diff
 import React, { Component } from 'react';
...
Finally, let's include the `BrowserRouter` so that all our routing can work by updating `src/index.js`:
```diff
 import React from 'react';
...
Now when you view the app, you can click on any of the nav items in the `NavigationDrawer` to see it animate between routes
