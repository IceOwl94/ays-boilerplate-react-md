...
Since `create-react-app` supports Sass and SCSS by default in v2, the steps for adding react-md to your app will be very simple. First, follow the steps outlined
in [adding a Sass Stylesheet](#adding-a-sass-stylesheet). Once `node-sass` has been installed, the `.css` files were renamed to `.scss`, and the `src/index.js` and
`src/App.js` were updated to import the `.scss` files, you can install `react-md`.
